...
export function KpiDock({ stats }: KpiDockProps) {
  const items: { label: string; value: number; color: string }[] = [
    ...
    { label: "Críticos", value: stats.totalCritical, color: "var(--red)" },
    { label: "Resueltos hoy", value: stats.resolvedToday, color: "var(--green)" },
  ...
  return (
    <div className="fixed left-5 top-[78px] z-10 flex w-[190px] flex-col gap-2">
      {items.map((item) => (
        <div key={item.label} className="glass-panel px-3.5 py-3">
